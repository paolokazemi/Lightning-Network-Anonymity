...
# USAGE: python3 execute.py <random | other> <num_transactions | 1000>

# Example: python3 execute.py random 1000
#   This will execute 1000 random transactions on the snapshot

# Example: python3 execute.py file
#   This will execute the transactions found in the file results/from.json again
...
# Saves results to file every transaction
...
if len(sys.argv) > 1 and sys.argv[1] == 'random':
    ...
    while (i < int(sys.argv[2]) if len(sys.argv) > 2 else 1000):
        u = -1
        v = -1
        ...
        # Random hops are added after having chosen an optimal path
        path, delay, amount = md.add_random_hops(G1, path, amt)
        if (len(path) > 0):
    ...
    for t in from_transactions:
        # Random hops are added assuming the set of transaction contains only optimal ones
        path, delay, amount = md.add_random_hops(G1, t['path'], t['amount'])
        if (len(path) > 0):
            T, transaction = route(G1, path, delay, amount, ads, t['amount'], file, G1.nodes[t['sender']]["Tech"])
            writeTo(file, transaction)
        if len(path) > 2:
            print(i, path, "done")
            i += 1
